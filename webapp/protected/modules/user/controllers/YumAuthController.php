...
class YumAuthController extends YumController {
	...
	public function actionLogin() {
		...
		if (isset($_POST['YumUserLogin'])) {
			...
			if ($success instanceof YumUser) {
				...
				//added by ajay for checking membership
				// check for the user role to be that of the user(2) and then check if there is any entry in the membership column
				// if there is no entry then take to membership page otherwise to redirectuser as normal.
	...
	public function redirectUser($user) {
		...
		if(Yii::app()->user->isAdmin()==false)
		    Yii::app()->user->setReturnUrl(Yii::app()->createUrl('/reservation/reservation'));
