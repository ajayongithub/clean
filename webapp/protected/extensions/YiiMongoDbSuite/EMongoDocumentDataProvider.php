...
class EMongoDocumentDataProvider extends CDataProvider
{
	...
	public static function  getInstance($modelClass, $params=array(), $options=array()){
		...
		if(isset($params[$modelClass]))
			$model->setAttributes($params[$modelClass]);
	    ...
	    if(isset($params['sort'])){
	    	$options['sort'] = array($params['sort']=>-1);
	    }
	    	
	   if(!$options['sort'] ){
	    	$options['sort'] = array('dateCreated'=>-1);
	    }
	    **/
	    
			
		$dataProvider= new EMongoDocumentDataProvider($model->search(), $options);
	...
	{
		//Yii::log(get_class($this)."   model:".CVarDumper::dumpAsString($modelClass),CLogger::LEVEL_ERROR);
		...
		Yii::log("findAll.criteria.native _criteria:".CVarDumper::dumpAsString($this->_criteria) , CLogger::LEVEL_INFO);
		...
		Yii::log("findAll.criteria.native _criteria.withdefault:".CVarDumper::dumpAsString($this->_criteria) , CLogger::LEVEL_INFO);
	...
	protected function fetchData()
	{
		if(($pagination=$this->getPagination())!==false)
		{
			...
			Yii::log("EmongoCriteria.findAll.criteria.native afterPagination:".CVarDumper::dumpAsString($this->_criteria) , CLogger::LEVEL_INFO);
